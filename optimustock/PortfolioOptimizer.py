...
class MarkowitzPortfolioOptimizer:
    ...
    def portfolio_optimizer_positive_weights(self):
        ...
        for rt in self.returns:
            ...
            optimalvol = minimize(
                self.get_volatility, w0, bounds=bounds, constraints=constraints2, method="SLSQP")
    ...
    def portfolio_optimizer_generalized(self):
        for rt in self.returns:
            wt = self.lagrange_solver(
                self.Sigma, self.expected_annual_return, rt)
    ...
    def fit(self, short_selling=False):
        ...
        output = dict({"volatility": volatility,
                      "weights": weights, "returns": np.round(returns, 2)})
